...
class Task:
    ...
    def __call__(self) -> None:
        ...
        try:
            ...
            route = submission.model_name
            # Support A/B routing: "nameA|nameB@p" selects A with probability p
            if "|" in route:
                try:
                    left, right_part = route.split("|", 1)
                    if "@" in right_part:
                        right, prob_str = right_part.split("@", 1)
                        p = float(prob_str)
                    else:
                        right = right_part
                        p = 0.5
                    chosen = left if random.random() < p else right
                    model_class = models[chosen]
                except Exception:
                    model_class = models[route]
            else:
                model_class = models[route]
            model = model_class()
